...
export default function Home() {
  const [currentScreen, setCurrentScreen] = useState<'mainMenu' | 'loading' | 'game' | 'guide'>('loading');
      ...
      return new Promise<void>((resolve) => {
        ...
        img.onload = () => {
          ...
          if (url.includes('laser-beam')) {
            console.log(`✅ Laser beam image loaded successfully: ${url}`);
          }
          resolve();
        };
        img.onerror = () => {
  ...
  const MainMenu = () => (
    <div className='min-h-screen bg-black flex flex-col w-full' style={{ 
      minHeight: '100vh', 
      backgroundColor: '#000', 
      display: 'flex', 
      flexDirection: 'column', 
      ...
      <div style={{ 
        ...
        paddingTop: 'clamp(60px, 12vw, 80px)' // Space for fixed top bar
      }}>
        {/* Enhanced Space Background - Matching Banner */}
        <div style={{ position: 'absolute', inset: 0, zIndex: 1, pointerEvents: 'none' }}>
          {/* Stars */}
          {Array.from({ length: 80 }).map((_, i) => {
            const size = Math.random() > 0.7 ? '3px' : Math.random() > 0.4 ? '2px' : '1px';
            ...
            return (
              <div
                ...
                style={{
                  ...
                  height: size,
                  opacity: intensity,
                  boxShadow: `0 0 ${size === '3px' ? '6px' : '3px'} rgba(255, 255, 255, ${intensity})`,
                  animation: twinkle ? 'twinkle 3s ease-in-out infinite alternate' : 'none'
                }}
              />
            );
          })}
          ...
          {Array.from({ length: 20 }).map((_, i) => (
            <div
              ...
              style={{
                position: 'absolute',
                backgroundColor: i % 3 === 0 ? '#00bfff' : i % 3 === 1 ? '#ffd700' : '#ff69b4',
                borderRadius: '50%',
                left: `${Math.random() * 100}%`,
                top: `${Math.random() * 100}%`,
                width: '1px',
                height: '1px',
                opacity: 0.4,
                boxShadow: `0 0 4px ${i % 3 === 0 ? '#00bfff' : i % 3 === 1 ? '#ffd700' : '#ff69b4'}`
              }}
            />
          ))}
          ...
          <div style={{
            ...
            width: '300px',
            height: '200px',
            background: 'radial-gradient(circle, rgba(0, 191, 255, 0.1) 0%, rgba(0, 80, 255, 0.05) 50%, transparent 100%)',
            borderRadius: '50%',
            filter: 'blur(40px)'
          }} />
          ...
          <div style={{
            ...
            width: '250px',
            height: '150px',
            background: 'radial-gradient(circle, rgba(255, 215, 0, 0.08) 0%, rgba(255, 140, 0, 0.04) 50%, transparent 100%)',
            borderRadius: '50%',
            filter: 'blur(35px)'
          }} />
        </div>
        ...
        <div style={{ 
          ...
          <div style={{
            ...
            width: '200px',
            height: '2px',
            background: 'linear-gradient(90deg, transparent, #00bfff, #ffffff, #00bfff, transparent)',
          ...
          <div style={{ 
            ...
            {['Progressive Bosses', '5-Tier Ship Upgrades', 'Cosmic Combat'].map((feature, index) => (
              <span
                key={feature}
                style={{
                  fontSize: 'clamp(11px, 2.2vw, 13px)',
                  color: index === 0 ? '#ffd700' : index === 1 ? '#00bfff' : '#ff69b4',
                  backgroundColor: 'rgba(0, 0, 0, 0.4)',
                  padding: '6px 12px',
                  borderRadius: '16px',
                  border: `1px solid ${index === 0 ? 'rgba(255, 215, 0, 0.3)' : index === 1 ? 'rgba(0, 191, 255, 0.3)' : 'rgba(255, 105, 180, 0.3)'}`,
                  backdropFilter: 'blur(10px)',
                  textShadow: `0 0 4px ${index === 0 ? '#ffd700' : index === 1 ? '#00bfff' : '#ff69b4'}`
                }}
              >
                {feature}
              </span>
        ...
        <button
          ...
          onMouseEnter={(e) => {
            if (assetsLoaded) {
              ...
              e.currentTarget.style.boxShadow = '0 0 40px rgba(255, 215, 0, 0.6), 0 8px 30px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.3)';
            }
          }}
          onMouseLeave={(e) => {
            if (assetsLoaded) {
              e.currentTarget.style.transform = 'scale(1) translateY(0)';
              e.currentTarget.style.boxShadow = '0 0 30px rgba(255, 215, 0, 0.4), 0 6px 20px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.2)';
            }
          }}
        >
